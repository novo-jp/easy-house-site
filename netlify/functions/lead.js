...
export default async (req, context) => {
  ...
  if (rateLimited(ip)) {
    return new Response(JSON.stringify({ error: 'Muitas tentativas. Aguarde um minuto.' }), { status: 429, headers: HEADERS });
  }

  let body;
  try { body = await req.json(); } catch {
    return new Response(JSON.stringify({ error: 'Corpo inválido' }), { status: 400, headers: HEADERS });
  }
  ...
  if (Object.keys(errors).length) {
    return new Response(JSON.stringify({ error: 'Dados incompletos', fields: errors }), { status: 422, headers: HEADERS });
  }
  ...
  try {
    ...
    return new Response(
      JSON.stringify({ ok: true, persisted: true, code: lead.code, leadId: lead.id, result }),
      { status: 200, headers: HEADERS }
    );
  } catch (err) {
    console.error('lead: falha ao gravar');
    return new Response(
      JSON.stringify({ ok: false, error: 'Não foi possível registrar agora. Fale conosco pelo WhatsApp.' }),
