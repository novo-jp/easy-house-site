...
export function runSimulation(input, config) {
  ...
  const term = calculateTerm({
    age: input.age,
    maximumTermYears: product.maximumTermYears,
  ...
  if (desired != null && incomeCapacity != null) {
    ...
    notes.push(
      'Este cenário foi calculado apenas a partir da parcela desejada. Ele não representa uma análise da sua capacidade de crédito.'
    );
  } else {
