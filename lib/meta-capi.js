...
export async function sendLeadEvent({ eventId, ads, ip, userAgent }) {
  return sendMetaEvent({ eventName: 'Lead', eventId, ads, ip, userAgent });
}

/**
 * Qualquer evento padrão do Meta pelo servidor, com o mesmo cuidado do Lead:
 * só cookies de atribuição, IP e navegador — nunca contato, nem em hash.
 * O /api/raiox usa para `Contact`, quando a pessoa vai ao WhatsApp com o
 * Raio-X pronto: é a conversão das landings, e o navegador sozinho perde uma
 * parte dela para bloqueadores e para o iOS.
 */
export async function sendMetaEvent({ eventName, eventId, ads, ip, userAgent }) {
