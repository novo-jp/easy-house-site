 ...
 *   - no máximo dois selos, só com dado real (pet conferido; sem luva/depósito);
...
(function (raiz, fabrica) {
  ...
  function cartao(a, o) {
    ...
    // ── selos: no máximo dois, e só com dado real
    var selos = [];
    if (a.pet === true) selos.push('<span class="selo selo--pet">Aceita pet</span>');
    if (!a.luva && !a.deposito && a.aluguel) selos.push('<span class="selo selo--zero">Sem luva e sem depósito</span>');
    else if (!a.luva && a.aluguel) selos.push('<span class="selo selo--zero">Sem luva</span>');
    ...
    if (a.endereco) acoes += '<a class="alu__sec" href="https://www.google.com/maps/search/' + encodeURIComponent(a.endereco) + '" target="_blank" rel="noopener" aria-label="Ver ' + esc(titulo) + ' no mapa">Mapa</a>';
    if (a.dkUrl) acoes += '<a class="alu__sec" href="' + esc(a.dkUrl) + '" target="_blank" rel="noopener" aria-label="Ver ' + esc(titulo) + ' no portal (abre em outra aba)">Portal <span aria-hidden="true">↗</span></a>';
