 ...
 * NFKC antes: os títulos vêm com letras de largura total ("Ｇａｒｄｅｎ") e
 * o portal escreve "Ⅰ" onde a pessoa digita "I". NFC depois: o NFD separa o
 * dakuten do kana (ヴ → ウ + ゛) e a chave tem que recompor para comparar.
 */
export const chave = (s) => String(s || '').normalize('NFKC').normalize('NFD').replace(/[̀-ͯ]/g, '').normalize('NFC').toLowerCase().trim();
