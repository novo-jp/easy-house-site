...
/**
 * Só aceita http(s) dos hosts que servem as fotos dos portais de origem.
 * Qualquer outra coisa vira null — o texto vem de terceiros e uma URL
 * arbitrária no `src` de uma imagem é vetor de rastreamento e de conteúdo
 * indevido.
 *
 * athome.jp: fonte atual (ATBB). miraie-net: fonte anterior (Reprice),
 * mantida enquanto houver registros dela em circulação.
 */
const HOSTS_FOTO = new Set([
  'img4.athome.jp', 'img.athome.jp', 'img1.athome.jp', 'img2.athome.jp', 'img3.athome.jp',
  'img.miraie-net.com', 'www.miraie-net.com', 'miraie-net.com',
  // Fichas geradas pela própria Easy House, para os imóveis cujo anúncio de
  // origem não permite republicar as fotos.
  'igtqdhesorahhdyvsjrl.supabase.co'
]);
...
const USO_TERRENO = {
  // O ATBB escreve com dígitos de largura total (１種住居); normalizamos
  // com NFKC antes de consultar, então basta a forma comum aqui.
  ...
  '工業地域': 'Industrial',
  '準工業地域': 'Misto industrial leve',
  '調整区域': 'Área de contenção urbana',
...
/**
 * Entrega como classe filtrável. O ATBB escreve "予定 2026年09月下旬" para obra
 * com data prevista, então a classe vem antes da tradução literal.
 *   imediata  → 即時
 *   prevista  → 予定 …
 *   combinar  → 相談 / 応相談 / 契約後
 */
export function classeEntrega(bruto) {
  const s = texto(bruto);
  if (!s) return null;
  if (/^即時/.test(s)) return 'imediata';
  if (/予定/.test(s)) return 'prevista';
  if (/相談|契約後/.test(s)) return 'combinar';
  return null;
}

/** "予定 2026年09月下旬" → "previsão: fim de setembro de 2026" (só quando dá para ler). */
export function entregaPrevistaPt(bruto) {
  const s = texto(bruto);
  if (!s) return null;
  const m = s.match(/(\d{4})年\s*(\d{1,2})月\s*(上旬|中旬|下旬)?/);
  if (!m) return null;
  const MESES = ['janeiro','fevereiro','março','abril','maio','junho','julho','agosto','setembro','outubro','novembro','dezembro'];
  const parte = { '上旬': 'início de ', '中旬': 'meados de ', '下旬': 'fim de ' }[m[3]] || '';
  return `previsão: ${parte}${MESES[Number(m[2]) - 1]} de ${m[1]}`;
}
...
function traduzirPor(dicionario, bruto) {
  const s = texto(bruto);
  if (!s) return null;
  // NFKC transforma １種住居 em 1種住居: as duas fontes escrevem diferente.
  const chave = s.normalize('NFKC');
  if (dicionario[chave]) return { pt: dicionario[chave], jp: s };
  if (dicionario[s]) return { pt: dicionario[s], jp: s };
  for (const [jp, pt] of Object.entries(dicionario)) {
    if (chave.includes(jp)) return { pt, jp: s };
  }
...
export function acesso(bruto, minutosBanco) {
  ...
  //   Reprice: 武豊線「亀崎」駅 3km 徒歩38分
  //   ATBB:    名古屋臨海高速鉄道 稲永 徒歩18分
  // No do ATBB a estação não vem entre colchetes: é a palavra antes do
  // 徒歩/バス/距離, e o que vem antes dela é o nome da linha.
  let est = s.match(/「([^」]+)」\s*駅/);
  let linha = s.match(/^([^「]+?)「/);
  if (!est) {
    const m = s.match(/^(.+?)\s+(\S+?)\s+(?:徒歩|バス|距離)/);
    if (m) { linha = [null, m[1]]; est = [null, m[2]]; }
  }
...
const DESTAQUES_PT = {
  ...
  // ── Termos do ATBB (fonte atual) ──────────────────────────
  'ディンプルキー': 'Fechadura de alta segurança',
  'カードキー': 'Fechadura por cartão',
  '外壁サイディング': 'Revestimento externo em siding',
  '浄水器': 'Filtro de água',
  '人感センサー付照明': 'Luz com sensor de presença',
  'シャワー付洗面化粧台': 'Bancada com ducha',
  'トイレ２ヶ所': '2 banheiros',
  'ダウンライト': 'Luminárias embutidas',
  '照明器具': 'Luminárias inclusas',
  '３口以上コンロ': 'Fogão de 3 bocas ou mais',
  '火災警報器（報知機）': 'Detector de incêndio',
  '都市ガス': 'Gás encanado',
  'プロパンガス': 'Gás de botijão',
  '下水道': 'Rede de esgoto',
  '浄化槽': 'Fossa séptica',
  '上水道': 'Água encanada',
  '電気': 'Energia elétrica',
  'シャッター雨戸': 'Persiana de proteção',
  '駐車場２台分': 'Vaga para 2 carros',
  '駐車場３台分以上': 'Vaga para 3 carros ou mais',
  '全居室フローリング': 'Piso de madeira em todos os quartos',
  '全室２面採光': 'Luz natural por 2 lados em todos os cômodos',
  'シューズＩＣ': 'Sapateira de entrada',
  '浴室に窓あり': 'Banheiro com janela',
  '省エネ給湯器': 'Aquecedor econômico',
  'バス１坪以上': 'Banheiro amplo',
  'Low-Eガラス': 'Vidro Low-E (isolante)',
  'クローゼット': 'Closet',
  'セットバック済': 'Recuo já regularizado',
  'ＩＨクッキングヒーター': 'Cooktop por indução',
  'スケルトンインフィル': 'Estrutura com planta flexível',
  '設計住宅性能評価書': 'Certificado de desempenho (projeto)',
  '建設住宅性能評価書': 'Certificado de desempenho (obra)',
  'オール電化': 'Totalmente elétrica',
  'オープンキッチン': 'Cozinha americana',
  'オートバス': 'Banheira automática',
  'クッションフロア': 'Piso vinílico',
  '省エネ設備': 'Equipamentos econômicos',
  '室内物干し': 'Varal interno',
  'パントリー収納': 'Despensa',
  'タンクレストイレ': 'Vaso sanitário sem caixa acoplada',
  'シャワールーム': 'Box de chuveiro',
  '２口コンロ': 'Fogão de 2 bocas',
  '出窓': 'Janela em sacada (bay window)',
  '側溝': 'Canaleta de drenagem',
  '蓄電池付き': 'Com bateria de armazenamento',
  'ＩＴ重説対応物件': 'Aceita explicação contratual on-line',
  '可動間仕切り': 'Divisória móvel',
  'ウォークスルークローゼット': 'Closet de passagem',
  'EV車充電設備': 'Ponto de recarga para carro elétrico',
  'インターネット対応': 'Preparada para internet',
  'セットバック要': 'Exige recuo do terreno',
  'ダブルロックドア': 'Porta com fechadura dupla',
  'デザイナーズ': 'Projeto assinado por arquiteto',
  'ハイサッシ採用': 'Esquadrias de pé-direito alto',
  '省令準耐火構造': 'Estrutura semirresistente a fogo',
  '長期優良住宅認定': 'Certificada como casa de longa durabilidade',
  'フットライト': 'Luz de piso',
  '二世帯向き': 'Adequada para duas famílias',
  '免震構造': 'Estrutura com isolamento sísmico',
  '制震構造': 'Estrutura com amortecimento sísmico',
  '分譲地内': 'Em loteamento planejado',
  '区画整理地内': 'Em área de reordenamento urbano',
  '天井高２.５ｍ以上': 'Pé-direito de 2,5 m ou mais',
  '床暖房': 'Piso aquecido',
  '庭10坪以上': 'Quintal de 33 m² ou mais',
  '新築時・増改築時の設計図書': 'Com plantas da obra e das reformas',
  '間接照明': 'Iluminação indireta',
  '食器乾燥機': 'Secadora de louças',
  '高気密高断熱住宅': 'Casa de alta vedação e isolamento térmico',
  '２WAYキッチン': 'Cozinha com dois acessos',
  '２×４工法': 'Construção em sistema 2x4',
  '２面採光': 'Luz natural por 2 lados',
};
...
export function formatarYen(v) {
  ...
  // Locale do público (pt-BR): ¥29.800.000. O mercado japonês usa vírgula,
  // mas quem lê é brasileiro — e o 万円 aparece ao lado, para quem negocia.
...
export function codigoPublico(linha) {
  ...
  if (cgi) return `EH-${cgi}`;                 // Reprice
  const id = texto(linha.id) || '';
  return `EH-${id.slice(-7).toUpperCase()}`;   // ATBB: fim do bukken_no
}
...
export function normalizarCasa(linha, casasConfig, financingConfig, agora = new Date()) {
  ...
  return {
    ...
    // Quando o anúncio de origem não libera as fotos, a imagem é a ficha da
    // Easy House. A interface precisa dizer isso: quem vê uma miniatura de
    // documento onde esperava a fachada merece saber por quê.
    imagemEhFicha: linha.sem_foto_original === true,

    estimativa: est,

    // Derivados para filtro e para o card. Calculados aqui, uma vez, para a
    // API, a página renderizada no servidor e o navegador dizerem a mesma coisa.
    construcaoNova: linha.construcao_nova === true,
    entregaClasse: classeEntrega(linha.entrega),
    entregaPrevista: entregaPrevistaPt(linha.entrega),
    // true = anúncio diz que tem vaga; false = diz que não tem; null = não informou.
    // O NÚMERO de vagas só existe em 1% dos anúncios, por isso não é filtro.
    temEstacionamento: vaga === null ? null : (vaga.vagas === 0 ? false : (vaga.vagas !== null || /有/.test(vaga.jp) ? true : null)),
    situacaoJp: texto(linha.situacao),
    pavimentosJp: texto(linha.pavimentos),
    // Data em que o scraper viu o anúncio na origem pela última vez. É o que
    // sustenta "disponibilidade verificada em …" — nunca "disponível agora".
    verificadoEm: (linha.last_seen_at && Number.isFinite(new Date(linha.last_seen_at).getTime()))
      ? new Date(linha.last_seen_at).toISOString().slice(0, 10) : null,
    fonteRotulo: (texto(linha.fonte) || '') === 'atbb' ? 'ATBB (at home)' : (texto(linha.fonte) || null),

    // Controle — usado pelo site, não mostrado como dado do imóvel
    origem: { fonte: texto(linha.fonte) || 'atbb', id: texto(linha.id), url: texto(linha.bukken_url) },
